...
class GraphMultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        # Rows that are all -inf (e.g. padded joints) -> set to 0 so softmax is well-defined.
        ...
        if self.value_emb_flag:
            ...
            value_hop_att = torch.scatter_add(
                value_hop_att,
                3,
                distance.unsqueeze(1).repeat(1, self.nheads, 1, 1),
                attn,
            )
            ...
            value_edge_att = torch.scatter_add(
                value_edge_att,
                3,
                edge_attr.unsqueeze(1).repeat(1, self.nheads, 1, 1),
                attn,
            )
