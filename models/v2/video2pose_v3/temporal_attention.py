...
def apply_rope_1d_perjoint(q: torch.Tensor, k: torch.Tensor):
    """
    q, k: [BJ, H, T, Dh] — per-joint 1D RoPE along time.
    """
    BJ, H, T, Dh = q.shape
...
class TemporalPerJointMultiHeadAttention(nn.Module):
    """
    Per-joint temporal attention with sliding-window visibility and RoPE.

    Each joint index is treated as an independent token sequence along time.
    A relative-time learned bias and a symmetric window mask restrict
    ...
    def forward(self, q, k, v, mask=None):
        ...
        q = self.linear_q(q).view(B * J, T, self.nheads, self.att_size).transpose(1, 2)
...
class TemporalPerJointTransformerBlock(nn.Module):
    """
    Pre-norm transformer block wrapping TemporalPerJointMultiHeadAttention
