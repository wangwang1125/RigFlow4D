    ...
    def __call__(
        ...
        if joint_mask is not None and encoder_hidden_states is hidden_states:
            ...
            if att_slidwindow is not None:
                ...
                # (1) pad joint_mask to align with key/value padding
                joint_mask_pad = Func.pad(joint_mask, (0, 0, pad, pad), value=False)

                # (2) construct sliding window indices (same as key/value)
                idx = torch.arange(seq_len, device=joint_mask.device)
                win_idx = idx.unsqueeze(1) + torch.arange(win, device=joint_mask.device)

                # (3) extract joint_mask inside window -> [B, F, win, J]
                joint_mask_win = joint_mask_pad[:, win_idx, :]

                # (4) reshape window joint_mask
                joint_mask_win = joint_mask_win.reshape(B * F, 1, 1, win * J)

                # (6) expand to multi-head & flatten
                mask = joint_mask_win.repeat(1, heads, J, 1)

                # (7) apply mask
                attn_scores = attn_scores.masked_fill(~mask, float('-inf'))

            # === normal (non-sliding window) mode ===
            else:
                # [B, F, J] → [B*F, heads, J, J]
...
class SimpleAttnProcessor:
    """Standard attention (no sliding window / RoPE), supports joint_mask"""
    ...
    def __call__(self, attn, hidden_states, encoder_hidden_states=None,
        ...
        if joint_mask is not None and encoder_hidden_states is None:
            # only used in self-attention (N=J)
